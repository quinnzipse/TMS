...
@section('content')
    <div class="container-fluid">
        ...
        <nav class="nav nav-pills nav-fill mt-2 mb-2">
            <a class="nav-link {{$callerID == 0 ? 'active' : ''}}" href="{{route('tasks')}}">Active</a>
            <a class="nav-link {{$callerID == 1 ? 'active' : ''}} " href="{{route('tasks.overdue')}}">Overdue</a>
            <a class="nav-link {{$callerID == 2 ? 'active' : ''}}" href="{{route('tasks.completed')}}">Completed</a>
        </nav>
            ...
            @foreach($tasks as $t)
                <tr>
                    ...
                    <td class="mr-0 pr-1 pl-1" colspan="{{$t->in_use ? '0' : '1'}}" {{$t->in_use ? 'hidden' : ''}}>
                        <button type="button" onclick="editTask({{$t->id}})"
                                class="btn btn-outline-primary btn-sm oBut{{$t->id}}"><i
                                class="far fa-edit"></i></button>
                    </td>
                    <td class="ml-0 mr-0 pr-1 pl-1" colspan="{{$t->in_use ? '0' : '1'}}" {{$t->in_use ? 'hidden' : ''}}>
                        <button type="button" onclick="markAsDone({{$t->id}})"
                                class="btn btn-outline-success btn-sm oBut{{$t->id}}"><i
